<?php include('inc/header.php') ?>
<!-- Sub banner start -->
<div class="sub-banner overview-bgi">
    <div class="container">
        <div class="breadcrumb-area">
            <h1>Experiencia en Desarrollos

            </h1>
            <p>Conoce lo que hemos logrado en nuestros más de 50 años</p>        
        </div>
    </div>
</div>
<!-- Sub banner end -->

<div class="timeline">
  <div class="year">
    <div class="inner">
      <span>Departamentos</span>
    </div>
  </div>

    <ul class="days">


        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/doscantos.jpg" alt="" />
            <p class="caption">
            2015
            </p>
            </div>
            <div class="date">Dos Cantos<br> San Pedro, N.L.</div>
        </div>
        </li>

        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/amazonas.jpg" alt="" />
            <p class="caption">
            2014
            </p>
            </div>
            <div class="date">Torre Amazonas<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/patagonia.jpg" alt="" />
            <p class="caption">
            2012
            </p>
            </div>
            <div class="date">Torres Patagonia<br> Monterrey, N.L.</div>
        </div>
        </li>

    </ul>

     <div class="year">
    <div class="inner">
      <span>Locales Comerciales</span>
    </div>
  </div>

    <ul class="days">


        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/volga.jpg" alt="" />
            <p class="caption">
            2013
            </p>
            </div>
            <div class="date">Plaza Volga<br> San Pedro</div>
        </div>
        </li>

        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/california.jpg" alt="" />
            <p class="caption">
            2012
            </p>
            </div>
            <div class="date">Plaza California<br> Contry, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/leoneselite.jpg" alt="" />
            <p class="caption">
            2011
            </p>
            </div>
            <div class="date">Plaza Leones Elite<br> Monterrey, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/sn1.jpg" alt="" />
            <p class="caption">
            2010
            </p>
            </div>
            <div class="date">Plaza S/N<br> Monterrey, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/sn2.jpg" alt="" />
            <p class="caption">
            2010
            </p>
            </div>
            <div class="date">Plaza S/N<br> Monterrey, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/sn3.jpg" alt="" />
            <p class="caption">
            2010
            </p>
            </div>
            <div class="date">Plaza S/N<br> Monterrey, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/351.jpg" alt="" />
            <p class="caption">
            2009
            </p>
            </div>
            <div class="date">Plaza 351<br> Contry, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/musas.jpg" alt="" />
            <p class="caption">
            2009
            </p>
            </div>
            <div class="date">Plaza Muzas<br> Contry, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/palmas.jpg" alt="" />
            <p class="caption">
            2008
            </p>
            </div>
            <div class="date">Plaza Palmas<br> Contry, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/americas.jpg" alt="" />
            <p class="caption">
            2007
            </p>
            </div>
            <div class="date">Las Américas<br> Contry, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/acapulco.jpg" alt="" />
            <p class="caption">
            2006
            </p>
            </div>
            <div class="date">Plaza Acapulco<br> Contry, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/constelaciones.jpg" alt="" />
            <p class="caption">
            2006
            </p>
            </div>
            <div class="date">Constelaciones<br> Monterrey, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/sn4.jpg" alt="" />
            <p class="caption">
            2004
            </p>
            </div>
            <div class="date">Plaza S/N<br> Monterrey, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/delparque.jpg" alt="" />
            <p class="caption">
            2003
            </p>
            </div>
            <div class="date">Del Parque<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/valle.jpg" alt="" />
            <p class="caption">
            2003
            </p>
            </div>
            <div class="date">Plaza Valle<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/lomas.jpg" alt="" />
            <p class="caption">
            2002
            </p>
            </div>
            <div class="date">Plaza Lomas<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/misiones.jpg" alt="" />
            <p class="caption">
            2002
            </p>
            </div>
            <div class="date">Plaza Las Misiones<br> San Nicolás, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/nogales.jpg" alt="" />
            <p class="caption">
            2002
            </p>
            </div>
            <div class="date">PLaza Los Nogales<br> Monterrey, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/miravalle.jpg" alt="" />
            <p class="caption">
            2001
            </p>
            </div>
            <div class="date">Plaza Miravalle<br> Monterrey, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/mississipi.jpg" alt="" />
            <p class="caption">
            2002
            </p>
            </div>
            <div class="date">Plaza Mississipi<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/vista2.jpg" alt="" />
            <p class="caption">
            1996
            </p>
            </div>
            <div class="date">Plaza Vista II<br> Cumbres, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/vista1.jpg" alt="" />
            <p class="caption">
            1996
            </p>
            </div>
            <div class="date">Plaza Vista I<br> Cumbres, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/sanangel.jpg" alt="" />
            <p class="caption">
            1995
            </p>
            </div>
            <div class="date">Plaza San Ángel<br> Cumbres, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/sn5.jpg" alt="" />
            <p class="caption">
            1994
            </p>
            </div>
            <div class="date">Plaza S/N<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/londres.jpg" alt="" />
            <p class="caption">
            1990
            </p>
            </div>
            <div class="date">Plaza Londres<br> Monterrey, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/moll.jpg" alt="" />
            <p class="caption">
            1980
            </p>
            </div>
            <div class="date">Moll del Valle<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/sn6.jpg" alt="" />
            <p class="caption">
            1977
            </p>
            </div>
            <div class="date">Plaza S/N<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/sn7.jpg" alt="" />
            <p class="caption">
            1975
            </p>
            </div>
            <div class="date">Plaza S/N<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/delbosque.jpg" alt="" />
            <p class="caption">
            1975
            </p>
            </div>
            <div class="date">Plaza Del Bosque<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/sn8.jpg" alt="" />
            <p class="caption">
            1970
            </p>
            </div>
            <div class="date">Plaza S/N<br> San Pedro, N.L.</div>
        </div>
        </li>
        <div class="year">


    <div class="inner">
      <span>Oficinas</span>
    </div>
  </div>

    <ul class="days">


        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/california.jpg" alt="" />
            <p class="caption">
            2012
            </p>
            </div>
            <div class="date">Plaza California<br> Contry, N.L.</div>
        </div>
        </li>

        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/oficinas-mg.jpg" alt="" />
            <p class="caption">
            2004
            </p>
            </div>
            <div class="date">Plaza MGM<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/oficinas-valle.jpg" alt="" />
            <p class="caption">
            2003
            </p>
            </div>
            <div class="date">Plaza Valle<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/oficinas-miravalle.jpg" alt="" />
            <p class="caption">
            2001
            </p>
            </div>
            <div class="date">Plaza Miravalle<br> Monterrey, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/oficinas-sn3.jpg" alt="" />
            <p class="caption">
            1994
            </p>
            </div>
            <div class="date">Plaza S/N<br> San Pedro, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/oficinas-londres.jpg" alt="" />
            <p class="caption">
            1990
            </p>
            </div>
            <div class="date">Plaza Londres<br> Monterrey, N.L.</div>
        </div>
        </li>
        <li class="day">
        <div class="events">
            <div class="day__img">
            <img src="https://alphamedia.sfo2.cdn.digitaloceanspaces.com/wel/corp/experiencia/oficinas-moll.jpg" alt="" />
            <p class="caption">
            1980
            </p>
            </div>
            <div class="date">Moll del Valle<br> San Pedro, N.L.</div>
        </div>
        </li>

    </ul>







    </ul>

    

    
  
  <div class="year year--end">
    <div class="inner">
     
    </div>
  </div>
  


  
</div>


<?php include('inc/footer.php') ?>